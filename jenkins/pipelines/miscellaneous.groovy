def shscript(illumosci, script, returnStdout, parameters) {
    def ret = null

    def environment = [
        "ILLUMOSCI_DIRECTORY=${illumosci}",
        "SH_LIBRARY_PATH=${illumosci}/jenkins/sh/library"
    ]
    ...
    withEnv(environment) {
        wrap([$class: 'AnsiColorBuildWrapper']) {
            ret = sh(encoding: 'UTF-8', returnStatus: false, returnStdout: returnStdout,
                script: "${illumosci}/jenkins/sh/${script}/${script}.sh")
        }
    }
